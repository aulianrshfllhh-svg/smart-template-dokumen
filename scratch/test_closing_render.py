...
def test_full_flow():
    ...
    out = 'storage/app/temp_docs/test_closing_format.docx'
    pdf_out = 'storage/app/temp_docs/test_closing_format.pdf'
    ...
    # 6. Clean Old Signature from End of Document
    ...
    # Anti-orphan for BAB Penutup content:
    # Set keep_with_next = True pada paragraf terakhir isi BAB Penutup
    if len(doc.paragraphs) > 0:
        doc.paragraphs[-1].paragraph_format.keep_with_next = True

    # 8. Inject Official Closing Format (BUPATI CIREBON & IMRON)
    ...
    p_bupati.paragraph_format.space_after = Pt(72) # Ruang tanda tangan
    ...
    # 9. Format Tables (Fit to Printable Width)
    for tbl in doc.tables:
        tbl_idx = list(body).index(tbl._tbl)
        tbl_sect_idx = 0
        for s_i, end_b_idx in enumerate(sect_body_indices):
